...
wb1 = load_workbook(filename = 'Derivatives Excel File.xlsx')
sheet = wb1.active

# Loads destination workbook
wb2 = Workbook()
dest_filename = 'DSF Tm Destination File.xlsx'
...
compound_count = int(input("Enter number of compounds to be tested:"))
titration_count = int(input("Enter number of titrations used:"))
conc_list = []
for conc_ind in range(titration_count):
    conc = round(float(input('Enter concentration {} of {}, in uM (highest to lowest, excluding apo):'.format(conc_ind + 1, titration_count))),2)
    conc_list.append(conc)
    # Allows user to fill in concentrations until required number of titrations is reached
conc_list.append(round(float(0), 2))
conc_count = len(conc_list)
row_count = 128
max_Tm_count = 5

tem_min = float(input('Enter temperature lower bound for Tm determination, in °C (default 50):'))
tem_max = float(input('Enter temperature upper bound for Tm determination, in °C (default 82):'))
resp_min = float(input('Enter response lower bound for Tm determination, in response units (default -5):'))
...
apo_col_indices = []
non_apo_col_indices = []
column_count = int(compound_count * titration_count + 16)   # 64 comps * 5 titrs + 16 apos = 336

for col_ind in range(column_count):     # 336 loops for 336 columns
    data_col_ind = (col_ind + 1) * 2    # Multiplies by 2 to skip the temperature columns
    column_name = sheet.cell(row = 1, column = data_col_ind).value
...
for col_ind in non_apo_col_indices:              # 320 loops for 320 non-apo columns
    if col_ind % (titration_count * 2) == 2:     # Takes the first of every 5 columns
        ...
        compound_names.append(column_name[-19:]) # Last 19 characters of string are the compound name
...
column_select = lambda comp_ind, conc_ind: (comp_ind * titration_count + conc_ind) if conc_ind != titration_count else "APO"
